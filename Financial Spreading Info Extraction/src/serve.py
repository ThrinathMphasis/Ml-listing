...
def upload():
    ...
    try:
        ...
        if path.exists("tmp/tika-server.log"):
            ...
            shutil.copy("src/tmp/tika-server.log", "/tmp/tika-server.log")
        else:
            shutil.copy("src/tmp/tika-server.log", "/tmp/tika-server.log")
        if path.exists("/tmp/tika-server.jar.md5"):
            os.remove("tmp/tika-server.jar.md5")
            shutil.copy("src/tmp/tika-server.jar.md5", "/tmp/tika-server.jar.md5")
        else:
            shutil.copy("src/tmp/tika-server.jar.md5", "/tmp/tika-server.jar.md5")
        if path.exists("/tmp/tika-server.jar"):
            os.remove("/tmp/tika-server.jar")
            shutil.copy("src/tmp/tika-server.jar", "/tmp/tika-server.jar")
        else:
            shutil.copy("src/tmp/tika-server.jar", "/tmp/tika-server.jar")
        if path.exists("/tmp/tika.log"):
            os.remove("/tmp/tika.log")
            shutil.copy("src/tmp/tika.log", "/tmp/tika.log")
        else:
            shutil.copy("src/tmp/tika.log", "/tmp/tika.log")
